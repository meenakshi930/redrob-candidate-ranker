...
def classify_title(title):
    title=title.lower()
    tech_keyword=["engineer","developer","programmer","software","machine learning","ml","ai",
                  "artificial intelligence","deep learning","dl","nlp","full stack",
                  "backend","frontend","devops","scientist","architect","researcher","technologist",
                  ]
    non_tech_keyword=["vice president","marketing","sales","business development",
                      "bd","hr","human resources","finance","accounting","recruiter",
                      "legal","lawyer","operations","admin","administrator",
                      ]  
        
    for keyword in non_tech_keyword:
        if keyword in title:
            return "Non-Tech"
...
bucket = {"Tech": 0, "Non-Tech": 0, "ambiguous": 0}
...
def classify_career(candidate):
    ...
    product_industries = [
        "Software", "Fintech", "Food Delivery",
        "E-commerce", "AI/ML", "Transportation"
    ]
...
for candidate in candidates:
    result = classify_career(candidate)
    print(candidate["profile"]["current_title"], "→", result)
...
def score_skill(candidate):
    ...
    for skill in candidate["skills"]:
        ...
        if matched_requirement is None:
            continue                                          # ← agla skill, neeche wala code skip

        base_points = weights[matched_requirement]

   
        if skill["name"] in assessment_scores:
            trust = assessment_scores[skill["name"]] / 100     # ← /100 missing tha
        else:
            trust = 0
...
def get_behavioral_multiplier(candidate):
    ...
    if candidate["redrob_signals"]["notice_period_days"]<=30:
        multiplier*=1.1
    elif candidate["redrob_signals"]["notice_period_days"]>90:
        multiplier*=0.8
...
for candidate in candidates[:50]:
    ...
    print(candidate["profile"]["current_title"], "→ final score:", round(score, 2))



with open(r"E:\redrob-candidate-ranker\candidates.jsonl", "rt", encoding="utf-8") as f:
...
with open("submission.csv", "w", newline="", encoding="utf-8") as f:
    ...
    rank = 1
    for score, cid, c in results[:100]:
        reasoning = (
